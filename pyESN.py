...
class ESN(BaseEstimator):
    ...
    def __init__(self, n_inputs: int, n_outputs: int, n_reservoir: int = 200,
                 spectral_radius: float = 0.95, sparsity: float =0, noise: float = 0.001,
                 ridge_noise: float = 0.001, wash_out: int = 100, learn_method: str ="ridge",
                 SGD_clf=None,
                 state_activation=np.tanh, leaky_rate: float = 1.0, is_SLM: bool = False,
                 intensity: float = 0.1, out_activation=identity, inverse_out_activation=identity,
                 W_in_scaling=None, W_feedb_scaling=None, input_shift=None, input_scaling=None, 
                 teacher_forcing=True, teacher_scaling=None, teacher_shift=None,
                 random_state=None, silent: bool =True):
        """
        Args:
        [Initialization]
            ...
            spectral_radius: spectral radius of the recurrent weight matrix in the reservoir
            ...
            ridge_noise: noise added in the ridge regression (regularization)
    ...
    def fit(self, inputs, outputs):
        ...
        transient = self.wash_out  # Wash-out Length (disregard the first few states)
        ...
        if self.learn_method == 'ridge':
            ...
            self.W_out = RLS_.w.T
        elif self.learn_method == 'batch':
            if self.W_out == None: # first iteration
                self.W_out = 0
        else:
